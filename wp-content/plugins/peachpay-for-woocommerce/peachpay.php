...
function peachpay_init() {
	...
	load_plugin_compatibility(
		array(
			...
			array(
				'plugin'        => 'sg-cachepress/sg-cachepress.php',
				'compatibility' => 'compatibility/sg-cachepress.php',
			),
			array(
				'plugin'        => 'wp-rocket/wp-rocket.php',
				'compatibility' => 'compatibility/wp-rocket.php',
			),
			array(
				'plugin'        => 'autoptimize/autoptimize.php',
				'compatibility' => 'compatibility/autoptimize.php',
			),
			array(
				'plugin'        => 'wp-optimize/wp-optimize.php',
				'compatibility' => 'compatibility/wp-optimize.php',
			),
			array(
				'plugin'        => 'litespeed-cache/litespeed-cache.php',
				'compatibility' => 'compatibility/litespeed-cache.php',
			),
			array(
...
function peachpay_email_us() {
	$body = array(
		...
		'salesLast12Months' => peachpay_sales_last_12_months(),
	);
...
function peachpay_authorize_url() {
	...
	$params       = array(
		...
		'callback_url' => peachpay_api_url( ( peachpay_is_staging_site() || peachpay_is_local_development_site() ) ? 'test' : 'prod' ) . 'store-token',
	);
...
function peachpay_send_deactivation_email() {
	...
	$body = array(
		...
		'stripe_connected'     => (bool) PeachPay_Stripe_Integration::connected(),
		'paypal_connected'     => (bool) PeachPay_PayPal_Integration::connected(),
		'salesLastMonth'       => peachpay_sales_last_month(),
		'salesYTD'             => peachpay_sales_ytd(),
	);

	peachpay_email( $body, 'api/v1/plugin/deactivation_email' );
}
