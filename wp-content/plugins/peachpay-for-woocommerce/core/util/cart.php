...
function peachpay_cart_tax_lines( $cart, $view = 'display' ) {
	// Edge case check for the order pay page.
	if ( ! method_exists( $cart, 'get_taxes_total' ) ||
		! method_exists( $cart, 'get_cart_contents_taxes' ) ||
		! method_exists( $cart, 'get_shipping_taxes' ) ||
		! method_exists( $cart, 'get_fee_taxes' ) ||
		! method_exists( $cart, 'get_tax_amount' ) ) {
			return;
	}
	...
	if ( 'itemized' !== get_option( 'woocommerce_tax_total_display' ) ) {
		return array(
			array(
				...
				'display' === $view ? 'amount' : 'total' => strval( $cart->get_taxes_total() ),
			),
		);
	}

	// Build itemized tax checking for all types of taxes.
	// Logic taken from /wp-content/plugins/woocommerce/includes/class-wc-checkout.php:create_order_tax_lines.
	$tax_list = array();
		...
		if ( $tax_rate_id && apply_filters( 'woocommerce_cart_remove_taxes_zero_rate_id', 'zero-rated' ) !== $tax_rate_id ) {
			array_push(
				...
				array(
					...
					'display' === $view ? 'amount' : 'total' => strval( $cart->get_tax_amount( $tax_rate_id ) ),
				)
			);
		}
	}
