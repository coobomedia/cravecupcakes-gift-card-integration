...
function peachpay_render_currency_notice() {
	...
	<div class='pp-currency-set-notice hide' style="<?php echo esc_html( $align ); ?>"> 
		...
		<?php
		esc_html_e( 'We detected the best currency for you would be ', 'peachpay_for_woocommerce' );
		...
		esc_html_e( ' and automatically converted the prices!', 'peachpay_for_woocommerce' );
		?>
		</div>
	</div>
	<?php
...
function peachpay_add_currency_fees( $cart ) {
	...
	if ( peachpay_get_settings_option( 'peachpay_currency_options', 'add_conversion_fees' ) && get_option( 'woocommerce_currency' ) !== $currency_cookie ) {
		...
		// phpcs:ignore
		if ( isset( $_POST['order'] ) && isset( $_POST['order']['payment_method'] ) ) {
			// phpcs:ignore
			$pm_name = explode( ':', $_POST['order']['payment_method'] )[0] ?? '';
		}
		// phpcs:ignore
		if ( isset( $_REQUEST['payment_method'] ) ) {
			// phpcs:ignore
			$pm_name = explode( '_', $_REQUEST['payment_method'] )[1] ?? '';
		}
		if ( ! $pm_name ) {
			return;
		}
		...
		if ( 'stripe' === $pm_name ) {
			$cart->add_fee( __( 'Currency Conversion', 'peachpay_for_woocommerce' ), $cart_total * 0.01 );
		} elseif ( 'paypal' === $pm_name ) {
			if ( 'USD' === $base_currency || 'CAD' === $base_currency ) {
				$cart->add_fee( __( 'Currency Conversion', 'peachpay_for_woocommerce' ), $cart_total * 0.04 );
			} else {
				$cart->add_fee( __( 'Currency Conversion', 'peachpay_for_woocommerce' ), $cart_total * 0.045 );
			}
		}
	}
