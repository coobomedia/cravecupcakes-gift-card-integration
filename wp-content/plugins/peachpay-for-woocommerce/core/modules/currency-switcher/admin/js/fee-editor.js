//GENERAL UTILITY:
/**
 * Selects all elements matching selectors and makes them hidden.
 * @param {*} selector element selector string to pass
 */
function pp_close_opened_elements(selector) {
...
/**
 * Handles click within the admin panel to allow fee editor menus to disappear when losing focus
 */
function pp_fee_editor_screen_click_event(event) {
    if (! event.target.closest('#pp-currency-rate-column')) {
        pp_close_opened_elements('.pp-fees-editor');
        return;
    }
}
...
window.addEventListener('load', () => {
    document.addEventListener('mousedown', pp_fee_editor_screen_click_event);
