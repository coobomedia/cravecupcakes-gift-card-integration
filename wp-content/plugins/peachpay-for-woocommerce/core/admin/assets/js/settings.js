...
function setupLeaveListener() {
	// This page does not behave like the others and is a little tricky to exclude
	// using the exception elements below, so it's easier to just exit for now.
	if (['product_links', 'home'].includes(params.get('tab')) || (params.get('page') === 'peachpay' && !params.get('tab'))) {
		return;
	}

	const listenForChangeForm = document.querySelector('#peachpay_settings_container');

	const allInputElements = Array.from(listenForChangeForm.querySelectorAll('select, input, textarea, tr:not(.pp-header)'));
	...
	const exceptionElements = Array.from(listenForChangeForm.querySelectorAll('input[name=select_field], .select-all, .see-more-state, #peachpay-register-landing'));
	...
	let submitButtons = document.querySelectorAll('#submit, .pp-pm-min-max-submit, button[type="submit"]');
