...
function peachpay_top_nav_dropdown() {
	...
	<div class="dropdown accordion-content">
		...
		<a class="<?php echo esc_attr( 'analytics' === peachpay_get_current_nav_top_tab() ? 'current' : '' ); ?>" href="<?php echo esc_url( PeachPay_Admin::admin_settings_url( 'peachpay_analytics', '', '', '', false ) ); ?>">
...
function peachpay_generate_nav_tab( $page, $tab, $section, $title, $has_subtabs = false ) {
	if ( $has_subtabs ) {
		?>
		<div class="tab-with-subtabs-container <?php echo esc_attr( peachpay_should_mark_premium( $tab ) ? 'pp-popup-trigger' : '' ); ?>">
			<div class="nav-tab has-subtabs accordion-tab <?php echo esc_attr( ( peachpay_get_current_nav_tab() === $tab ) ? 'current expanded' : '' ); ?>">
				...
				<?php if ( peachpay_should_mark_premium( $tab ) ) { ?>
					<div class="pp-popup pp-popup-right pp-tooltip-popup"> <?php esc_html_e( 'Premium Feature' ); ?> </div>
				<?php } ?>
			</div>
			<?php peachpay_get_subtabs( $tab ); ?>
		</div>
	<?php } else { ?>
		<a class="nav-tab <?php echo esc_attr( ( peachpay_get_current_nav_tab() === $tab ) ? 'current' : '' ); ?> <?php echo esc_attr( peachpay_should_mark_premium( $tab ) ? 'pp-popup-trigger' : '' ); ?>" href="<?php esc_url( PeachPay_Admin::admin_settings_url( $page, $tab, $section ) ); ?>">
			<div class="title">
				<div class="icon <?php echo esc_attr( str_replace( '_', '-', $tab ) ); ?>-icon">
				</div>
				<?php if ( peachpay_nav_feature_is_active( $tab ) ) { ?>
...
function peachpay_should_mark_premium( $tab ) {
	...
	return ! $peachpay_has_premium_capability && in_array(
		$tab,
		array(
			'currency',
			'field',
