...
if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
	class WPCleverWoosb {
									...
									<td>
										...
										<div class="woosb_show_if_woovr" style="margin-top: 10px">
											...
											<select name="woosb_settings[selector_interface]">
												...
												<option value="grid" <?php selected( $selector_interface, 'grid' ); ?>><?php esc_html_e( 'Grid', 'woo-product-bundle' ); ?></option>
											</select>
		...
		function mini_cart_item_visible( $visible, $cart_item ) {
			$parent_id = isset( $cart_item['woosb_parent_id'] ) ? absint( $cart_item['woosb_parent_id'] ) : 0;

			if ( ! apply_filters( 'woosb_item_visible', true, $cart_item['data'], $parent_id ) ) {
				return false;
			}

			if ( $parent_id && ( WPCleverWoosb_Helper::get_setting( 'hide_bundled_mini_cart', 'no' ) === 'yes' ) ) {
				return false;
			}
		...
		function cart_item_visible( $visible, $cart_item ) {
			...
			if ( $parent_id && ( WPCleverWoosb_Helper::get_setting( 'hide_bundled', 'no' ) !== 'no' ) ) {
				return false;
			}
		...
		function order_item_visible( $visible, $order_item ) {
			$parent_id = $order_item->get_meta( '_woosb_parent_id' ) ? absint( $order_item->get_meta( '_woosb_parent_id' ) ) : 0;
